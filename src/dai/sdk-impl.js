...
SdkImpl.prototype.requestStream = function() {
  ...
  if (streamType === 'VOD') {
    ...
    if (this.controller.getSettings().cmsId) {
      streamRequest.contentSourceId = this.controller.getSettings().cmsId;
    } else {
      window.console.warn(
        'VOD stream requests require setting the content source ID.');
    }
    if (this.controller.getSettings().videoId) {
      streamRequest.videoId = this.controller.getSettings().videoId;
    } else {
      window.console.warn(
        'VOD stream requests require setting the video ID.');
    }
  } else if (streamType === 'Live') {
    streamRequest = new google.ima.dai.api.LiveStreamRequest();
    if (this.controller.getSettings().assetKey) {
